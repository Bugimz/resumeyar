...
class PdfService {
  ...
  Future<Uint8List> generateResumePdf({
    ...
    final List<Certification> certifications =
        await certificationRepository.getAll();
    ...
    final filteredSectionOrder = sectionOrder
        .where((section) => !hiddenSections.contains(section))
        .toList();

    _currentPalette = _paletteFor(pdfTheme);
    ...
    final pageTheme = pw.PageTheme(
      ...
      buildBackground: (context) =>
          pw.Container(color: _currentPalette.background),
    );
  ...
  List<pw.Widget> _buildSectionsByOrder(
    ...
    for (var i = 0; i < sectionOrder.length; i++) {
      ...
      if (i != sectionOrder.length - 1) {
        widgets.add(_buildTemplateDivider(template));
      }
    }
  ...
  pw.Widget _buildTemplateDivider(ResumeTemplate template) {
    switch (template) {
  ...
  pw.Widget _buildInterestSection(List<Interest> interests, bool isRtl) {
    return pw.Column(
      ...
      children: [
        ...
        ...interests.map(
          (interest) => pw.Bullet(
            text: interest.description.isNotEmpty
  ...
  pw.Widget _buildSkillSection(List<Skill> skills, bool isRtl) {
    // گروه‌بندی skills بر اساس category (String)
    final Map<String, List<Skill>> categoryMap = {};

    for (final skill in skills) {
      if (!categoryMap.containsKey(skill.category)) {
        categoryMap[skill.category] = [];
      }
      categoryMap[skill.category]!.add(skill);
    }

    // مرتب‌سازی هر دسته بر اساس sortOrder
                    ...
                    child: pw.ClipRRect(
                      ...
                      child: pw.Container(
                        ...
                        child: pw.FittedBox(
                          ...
                          child: pw.Image(
                            projectImages[project.thumbnailUrl]!,
                          ),
                        ),
                      ),
                    ),
